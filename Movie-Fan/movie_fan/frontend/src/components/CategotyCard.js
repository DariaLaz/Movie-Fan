...
import { categoryPath, getPath, submitionPath } from "../Paths";
import { uploadPage, votePage } from "../RedirectPages";
...
export default function CategoryCard({ category }) {
  ...
  if (category.mode == 1) {
    (async () => {
      ...
      const response = await get(getPath(submitionPath, urlObj));
  ...
  if (category.mode == 2) {
    (async () => {
      ...
      const response = await get(getPath(categoryPath, urlObj));
      ...
      {mode == 3 && (
        <Typography align="center">
          ...
          <Button variant="contained" color="primary" onClick={handleShowMore}>
            {!showMore ? "Show Results" : "Hide Results"}
          </Button>
        </Typography>
      )}
      {showMore && (
